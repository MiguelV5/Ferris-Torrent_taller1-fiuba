...
/// ```
pub fn from_string(to_encode: Vec<u8>) -> String {
    to_encode
        .into_iter()
        .map(|ch| match ch {
...
mod tests {
    ...
    fn ascii_chars_encodes_ok() {
        let to_encode = String::from("abcdefABCDEF").as_bytes().to_vec();
        let result = from_string(to_encode);
    ...
    fn special_ascii_chars_encodes_ok() {
        let to_encode = String::from("$&+,/:;=?@").as_bytes().to_vec();
        let result = from_string(to_encode);
    ...
    fn unsafe_ascii_chars_encodes_ok() {
        let to_encode = String::from(" <>#%{}|^[]`").as_bytes().to_vec();
        let result = from_string(to_encode);
        assert_eq!(result, String::from("%20%3c%3e%23%25%7b%7d%7c%5e%5b%5d%60"));
    }

    #[test]
    fn mix_ascii_chars_encodes_ok() {
        let to_encode = String::from(" A<>d#%{}|^~[]RR`mpqZ").as_bytes().to_vec();
        let result = from_string(to_encode);
        assert_eq!(
            result,
            String::from("%20A%3c%3ed%23%25%7b%7d%7c%5e~%5b%5dRR%60mpqZ")
        );
    }
}
