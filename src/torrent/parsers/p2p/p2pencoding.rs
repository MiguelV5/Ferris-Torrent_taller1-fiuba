...
use super::{p2p_constants::*, p2p_message::*};
...
///
/// ```no_run
/// # use fa_torrent::torrent::parsers::p2p::p2pencoding::encode_p2p_message_to_bytes;
/// # use fa_torrent::torrent::parsers::p2p::p2p_message::*;
...
/// let request_bytes = encode_p2p_message_to_bytes(request);
/// // ... para posteriormente neviarle estos bytes a otro peer a traves de un socket.
/// ```
///
pub fn encode_p2p_message_to_bytes(p2p_msg: P2PMessage) -> Result<Vec<u8>, P2PMessageError> {
    match p2p_msg {
...
mod tests_p2p_encoder {
    ...
    mod tests_payloadless_encodings {
        ...
        fn encode_keep_alive_ok() {
            assert_eq!(
                Ok(vec![0; 4]),
                encode_p2p_message_to_bytes(P2PMessage::KeepAlive)
            )
        }
        ...
        fn encode_choke_ok() {
            assert_eq!(
                Ok(vec![0, 0, 0, 1, ID_CHOKE]),
                encode_p2p_message_to_bytes(P2PMessage::Choke)
            )
        }
        ...
        fn encode_unchoke_ok() {
            assert_eq!(
                ...
                encode_p2p_message_to_bytes(P2PMessage::Unchoke)
            )
        }
        ...
        fn encode_interested_ok() {
            assert_eq!(
                ...
                encode_p2p_message_to_bytes(P2PMessage::Interested)
            )
        }
        ...
        fn encode_not_interested_ok() {
            assert_eq!(
                ...
                encode_p2p_message_to_bytes(P2PMessage::NotInterested)
            )
        }
    }
    ...
    mod tests_have_encoding {
        ...
        fn encode_have_with_low_piece_index_ok() {
            ...
            assert_eq!(Ok(expected_bytes), encode_p2p_message_to_bytes(msg_to_send));
        }
