//! # Modulo de URLencoding
//! Modulo que contiene la función principal para realizar el encoding de Percent-encoding (URLencoding) a partir de bytes.
...
// let result = url_encoder::from_string_bytes(to_encode);
// ```
pub fn from_string_bytes(to_encode: Vec<u8>) -> Vec<u8> {
    to_encode
        .into_iter()
        .map(|ch| match ch {
...
mod tests_urlencoding_encoder {
    ...
    fn space_encodes_to_percent20_ok() {
        ...
        let result = from_string_bytes(space);
    ...
    fn ascii_control_characters_encodes_ok() {
        ...
        let to_encode = String::from_utf8_lossy(bytes)
            ...
            .as_bytes()
            .to_vec();
        let result = from_string_bytes(to_encode);
    ...
    fn bytes_encodes_ok() {
        ...
        let result = from_string_bytes(bytes.to_vec());
        assert_eq!(
            result,
